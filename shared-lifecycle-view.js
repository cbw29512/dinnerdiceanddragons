(() => {
  ...
  function actionButton(label, onClick, secondary = false) {
    try {
      ...
      logError("Unable to create lifecycle action", error);
      return make("span");
    }
  }
  ...
  function stateSummary(state) {
    try {
      return `${String(state.status || "forming").toUpperCase()} · Venue ${state.venue_approved ? "approved" : "pending"} · ${Number(state.confirmed_players || 0)}/${Number(state.min_players || 0)} Players needed · ${Number(state.requested_players || 0)} waiting for review · ${Number(state.waitlisted_players || 0)} waitlisted`;
    } catch (error) {
      logError("Unable to summarize game state", error);
  ...
  function card(title, eyebrow) {
    try {
      ...
      logError("Unable to create lifecycle card", error);
  ...
  function renderMessage(container, title, message) {
    try {
      ...
      logError("Unable to render lifecycle message", error);
    }
  }
  ...
  function renderGM(container, queue, manage) {
    try {
      container.replaceChildren();
      const intro = card(queue.game?.title || "Your Forming Table", "DUNGEON MASTER");
      ...
      (queue.registrations || []).forEach((registration) => {
        ...
        item.appendChild(make("p", registration.requested_at ? `Requested ${registration.requested_at}` : "Wants to join your table", "microcopy"));
        const actions = make("div", null, "cta-row");
        if (registration.status === "requested") {
          actions.appendChild(actionButton("Accept Player", () => manage(registration.registration_id, "approve")));
      ...
      if (!(queue.registrations || []).length) container.appendChild(card("No Player requests yet", "WAITING FOR PLAYERS"));
    } catch (error) {
      logError("Unable to render DM lifecycle", error);
    }
  }
  ...
  function renderVenue(container, queue, manage) {
    try {
      container.replaceChildren();
      const intro = card(queue.venue?.name || "Your Venue", "VENUE");
      intro.appendChild(make("p", "Review game-night booking requests for this venue.", "microcopy"));
      container.appendChild(intro);
      ...
      (queue.bookings || []).forEach((booking) => {
        ...
        item.appendChild(make("p", `${booking.expected_guests || 0} expected guests · DM ${booking.gm_display_name || "Dungeon Master"}`, "microcopy"));
        item.appendChild(make("p", `${String(booking.game_status || "forming").toUpperCase()} · ${booking.confirmed_players || 0} confirmed Players · ${booking.requested_players || 0} requests`, "microcopy"));
        const actions = make("div", null, "cta-row");
        if (booking.booking_status === "requested") {
          actions.appendChild(actionButton("Approve Booking", () => manage(booking.game_id, "approve")));
          actions.appendChild(actionButton("Decline", () => manage(booking.game_id, "decline"), true));
        } else if (booking.booking_status === "approved") {
          actions.appendChild(actionButton("Reopen Booking", () => manage(booking.game_id, "reopen"), true));
        } else if (booking.booking_status === "declined") {
          actions.appendChild(actionButton("Reconsider Booking", () => manage(booking.game_id, "reopen")));
        }
      ...
      logError("Unable to render Venue lifecycle", error);
    }
  }
  ...
  function renderPlayer(container, state, cancel) {
    try {
      ...
      (state.registrations || []).forEach((registration) => {
        ...
        item.appendChild(make("p", `${Number(game.confirmed_players || 0)}/${Number(game.min_players || 0)} Players needed · Venue ${game.venue_approved ? "approved" : "pending"}`, "microcopy"));
        const actions = make("div", null, "cta-row");
      ...
      if (!(state.registrations || []).length) container.appendChild(card("You have no active game registrations", "PLAYER"));
    } catch (error) {
      logError("Unable to render Player lifecycle", error);
    }
  }
