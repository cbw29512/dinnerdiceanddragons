(() => {
  ...
  const TYPE_LABELS = Object.freeze({
    ...
    event_disrupted: "Your Event needs attention",
    event_cancelled: "Event cancelled",
    event_changed: "Event details changed",
    attendance_reminder: "Game night reminder"
  });
  const EVENT_TYPES = new Set(["event_disrupted", "event_cancelled", "event_changed", "attendance_reminder"]);
  ...
  async function markRead(item) {
    ...
    } catch (error) { console.error("[DDD Notifications] Unable to mark notification read", error); }
  }

  function actionLink(item) {
    const payload = item.payload || {};
    if (payload.event_id && EVENT_TYPES.has(item.type)) {
      const link = element("a", "button primary", item.type === "attendance_reminder" ? "Open Game Night" : "Open Game Hub");
  ...
  function timingText(item) {
    try {
      if (item.type === "attendance_reminder") return item.payload?.starts_at ? `Game starts ${formatTime(item.payload.starts_at)}` : "Game night reminder";
      return item.expires_at ? `Respond by ${formatTime(item.expires_at)}` : "";
    } catch (error) {
      console.error("[DDD Notifications] Unable to format alert timing", error);
      return "";
    }
  }
  ...
  function renderItems(items) {
    ...
    for (const item of items) {
      ...
      card.append(element("p", "eyebrow", String(item.type).replaceAll("_", " ").toUpperCase()), element("h2", "", TYPE_LABELS[item.type] || "DDD notification"));
      const meta = element("div", "notification-meta");
      meta.append(element("span", "", formatTime(item.created_at)));
      const timing = timingText(item);
      if (timing) meta.append(element("span", "", timing));
      card.append(meta);
      const action = actionLink(item);
      if (action) { const actions = element("div", "notification-actions"); actions.append(action); card.append(actions); }
      list.append(card);
    }
  }
  ...
  async function savePause(current) {
    try {
      ...
      byId("preference-status").textContent = next.matching_paused ? "Matching paused. Your saved availability was not deleted." : "Matching active.";
      return next;
    } catch (error) {
  ...
  async function init() {
    try {
      ...
      if (!session) { byId("notification-status").textContent = "Sign in to see your DDD notifications."; return; }
      const [items, initialPrefs] = await Promise.all([window.DDDProductionAPI.getNotifications(), loadPreferences()]);
      renderItems(items || []);
