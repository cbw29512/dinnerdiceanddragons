(() => {
  ...
  function log(message, error) {
    console.error(`[DDD DM Account] ${message}`, error);
  }
  ...
  function setMode(form, mode) {
    try {
      ...
      displayLabel(form).hidden = signingIn;
  ...
  function enableDisplayName(form) {
    try {
      ...
      displayLabel(form).hidden = false;
    } catch (error) {
  ...
  async function authenticate(form, mode) {
    try {
      if (mode === "signup") {
        const nameCheck = valid(form, "display_name", "Enter the name Players should see.");
  ...
  window.DDDDMStartAccount = Object.freeze({
    authenticate,
    enableDisplayName,
    lockSignedIn,
    setMode,
