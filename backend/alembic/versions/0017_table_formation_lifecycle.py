"""create production table formation and Event lifecycle

Revision ID: 0017_table_formation_lifecycle
...
revision: str = "0017_table_formation_lifecycle"
...
def upgrade() -> None:
    """Create durable booking, series, Event, expectation, and registration state."""
    ...
    op.create_table(
        ...
        sa.Column("expected_sessions", sa.SmallInteger(), server_default="1", nullable=False),
        sa.Column("starts_on", sa.Date(), nullable=False),
        sa.Column("ends_on", sa.Date(), nullable=True),
        sa.Column("active", sa.Boolean(), server_default=sa.text("true"), nullable=False),
        sa.CheckConstraint("length(trim(title)) BETWEEN 1 AND 200", name="ck_game_series_title_length"),
        sa.CheckConstraint("expected_sessions >= 1", name="ck_game_series_expected_sessions"),
        sa.CheckConstraint("ends_on IS NULL OR starts_on <= ends_on", name="ck_game_series_date_order"),
        sa.ForeignKeyConstraint(["table_match_id"], ["table_matches.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(["gm_profile_id"], ["gm_profiles.id"], ondelete="RESTRICT"),
    ...
    op.create_table(
        ...
        sa.Column("beginner_friendly", sa.Boolean(), server_default=sa.text("true"), nullable=False),
        ...
        sa.ForeignKeyConstraint(["game_series_id"], ["game_series.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(["table_match_id"], ["table_matches.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(["gm_profile_id"], ["gm_profiles.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["game_system_id"], ["game_systems.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["venue_id"], ["venues.id"], ondelete="RESTRICT"),
    ...
    op.create_table(
        ...
        sa.Column("age_environment", sa.String(length=120), nullable=True),
        sa.Column("play_style", sa.Text(), nullable=False),
        sa.Column("boundaries", sa.Text(), nullable=False),
        sa.Column("pvp_policy", sa.String(length=300), nullable=True),
        sa.Column("homebrew_policy", sa.Text(), nullable=True),
        sa.Column("character_death_policy", sa.String(length=500), nullable=True),
        sa.Column("mature_content_notes", sa.Text(), nullable=True),
        sa.Column("alcohol_policy", sa.String(length=500), nullable=True),
        sa.Column("new_players_welcome", sa.Boolean(), server_default=sa.text("true"), nullable=False),
        sa.Column("break_policy", sa.String(length=500), nullable=True),
        sa.Column("safety_framework", sa.String(length=1000), nullable=True),
    ...
    op.create_table(
        ...
        sa.UniqueConstraint("event_id", "player_profile_id", name="uq_registrations_event_player"),
    )
    ...
    op.create_table(
        ...
        sa.Column("tables_requested", sa.SmallInteger(), server_default="1", nullable=False),
        sa.Column("expected_guests", sa.SmallInteger(), server_default="1", nullable=False),
        sa.Column("status", sa.String(length=16), server_default="requested", nullable=False),
        sa.Column("venue_message", sa.Text(), nullable=True),
        sa.Column("gm_message", sa.Text(), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.Column("updated_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
        sa.CheckConstraint("requested_end > requested_start", name="ck_venue_booking_requests_time_order"),
        sa.CheckConstraint("tables_requested >= 1", name="ck_venue_booking_requests_tables_requested"),
        sa.CheckConstraint("expected_guests >= 1", name="ck_venue_booking_requests_expected_guests"),
        sa.CheckConstraint(
            "status IN ('requested','question','approved','declined','cancelled')",
            name="ck_venue_booking_requests_status",
        ),
        sa.ForeignKeyConstraint(
            ["venue_table_window_id"], ["venue_table_windows.id"], ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(["gm_profile_id"], ["gm_profiles.id"], ondelete="RESTRICT"),
        sa.ForeignKeyConstraint(["table_match_id"], ["table_matches.id"], ondelete="SET NULL"),
        sa.ForeignKeyConstraint(["game_series_id"], ["game_series.id"], ondelete="SET NULL"),
        ...
        sa.UniqueConstraint("table_match_id", name="uq_venue_booking_requests_table_match_id"),
    ...
    op.create_index("ix_venue_booking_requests_gm_profile_id", "venue_booking_requests", ["gm_profile_id"])
    op.create_index("ix_venue_booking_requests_game_series_id", "venue_booking_requests", ["game_series_id"])
    op.create_index("ix_venue_booking_requests_requested_start", "venue_booking_requests", ["requested_start"])
...
def downgrade() -> None:
    """Remove Step 4 formation state in reverse dependency order."""
    ...
    op.drop_index(
        "ix_venue_booking_requests_venue_table_window_id",
        table_name="venue_booking_requests",
    )
