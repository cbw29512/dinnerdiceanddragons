...
def test_alembic_configuration_discovers_current_head() -> None:
    ...
    assert script.get_heads() == ["0017_table_formation"]
...
def test_alembic_offline_upgrade_emits_current_foundation_tables_without_database() -> None:
    ...
    assert "uq_game_series_recurring_rule_id" in result.stdout
    assert "CREATE TABLE events" in result.stdout
    assert "ck_events_player_range" in result.stdout
    ...
    assert "ck_venue_booking_requests_time_order" in result.stdout
    assert "uq_venue_booking_requests_event_id" in result.stdout
    ...
    for table_name in (
        ...
        assert f'ALTER TABLE public."{table_name}" ENABLE ROW LEVEL SECURITY' in result.stdout
    assert "deny_privileged_audit_event_mutation" in result.stdout
